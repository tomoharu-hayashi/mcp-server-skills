...
```bash
# 基本（引数なし = 自律モード）
copilot --agent=pm-agent --prompt "自律モードで開始して"
copilot --agent=engineer-agent --prompt "自律モードで開始して"

# Issue指定
copilot --agent=pm-agent --prompt "#<Issue番号> を進めて"
copilot --agent=engineer-agent --prompt "#<Issue番号> を進めて"

# 追加指示付き
copilot --agent=pm-agent --prompt "#<Issue番号> は 関連Issue と連携する前提。依存関係と受け入れ条件を整理して"
copilot --agent=pm-agent --prompt "#<Issue番号> は 他タスクより後でOK。優先順位の理由と判断材料を整理して"
copilot --agent=engineer-agent --prompt "#<Issue番号> は 関連Issue と連携しています。統合できているか確認して結果を共有して"
copilot --agent=engineer-agent --prompt "#<Issue番号> は 既存仕様に合わせた実装方針とリスクを短くまとめて"

# 条件付きコマンド（必要なときだけ）
copilot --agent=ship-agent --prompt "リリース可能なら実行して"
copilot --agent=ops-agent --prompt "運用・監視が不足しているので対応して"
copilot --agent=growth-agent --prompt "収益化・計測が未整備なので対応して"
```
