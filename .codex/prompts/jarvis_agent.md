...
```bash
# 基本（引数なし = 自律モード）
codex exec "/prompts:pm_agent"
codex exec "/prompts:engineer_agent"

# Issue指定
codex exec "/prompts:pm_agent #<Issue番号>"
codex exec "/prompts:engineer_agent #<Issue番号>"

# 追加指示付き
codex exec "/prompts:pm_agent #<Issue番号> 関連Issueと連携する前提。依存関係と受け入れ条件を整理して"
codex exec "/prompts:pm_agent #<Issue番号> 他タスクより後でOK。優先順位の理由と判断材料を整理して"
codex exec "/prompts:engineer_agent #<Issue番号> 関連Issueと連携しています。統合できているか確認して結果を共有して"
codex exec "/prompts:engineer_agent #<Issue番号> 既存仕様に合わせた実装方針とリスクを短くまとめて"

# 条件付きコマンド（必要なときだけ）
codex exec "/prompts:ship_agent" # リリース可能なとき
codex exec "/prompts:ops_agent" # 運用・監視が不足しているとき
codex exec "/prompts:growth_agent" # 収益化・計測が未整備のとき
```
