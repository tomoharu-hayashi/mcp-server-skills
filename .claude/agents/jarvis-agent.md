...
name: jarvis-agent
description: |
  Use this agent when you need JARVIS モード.
  <example>
  User: "Run jarvis-agent."
  Assistant: Use the Agent tool to run jarvis-agent.
  </example>
model: inherit
color: blue
---
...
Task ツールを使う（外部CLIは使わない）

```text
Task(subagent_type="pm-agent", prompt="...")
Task(subagent_type="engineer-agent", prompt="#<Issue番号> を完了せよ")
Task(subagent_type="ship-agent", prompt="...")      # 必要時のみ
Task(subagent_type="ops-agent", prompt="...")       # 必要時のみ
Task(subagent_type="growth-agent", prompt="...")    # 必要時のみ
```
